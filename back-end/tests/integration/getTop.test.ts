...
describe("POST /recommendations/top/:amount", () => {
    it("give a valid amount and that more than 'amount' recommendations are registered it should return the top 'amount' recommendations sorted by decrescent scores", async () => {
        ...
        expect(recommendations[randomIndex].score).toBeGreaterThanOrEqual(
            recommendations[randomIndex + 1].score
        );
    ...
    it("given a valid amount and that less than 'amount' recommendations are registered it should return all the recommendations sorted by decrescent scores", async () => {
        ...
        const randomMax = parseInt(
            faker.random.numeric(3, {allowLeadingZeros: false})
        );
        const amount = Math.floor(Math.random() * (randomMax - n + 1) + 1);
